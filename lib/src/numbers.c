...
/*   Updated: 2018/10/23 22:34:00 by cuzureau         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void		ft_add_number(t_field *f, t_main *m)
{
	...
	ft_get_number(&n, m, f);
	ft_prefix(&n, f);
	if (f->prec < (ft_slen(n.start) + ft_slen(n.prefix)))
	...
	if (!(f->flag & MINUS))
	{
		if (!(f->flag & ZERO) || (f->flag & ZERO && f->prec > 0))
			ft_fill(m, ' ', n.len);
		ft_add(n.prefix, ft_slen(n.prefix), m, f);
		if (f->prec > 0)
	...
	ft_add(n.start, ft_slen(n.start), m, f);
